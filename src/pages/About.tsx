...
const About = () => {
  ...
  const updateAbout = (refkey:string,title:string,text:string,setloading:Function)=>{
    setloading(true)
    updateDoc(doc(db,"Website/AboutPage/Contents",refkey),{title,text}).then(()=>{
      getAbouts()
      setloading(false)
    }).catch((err)=>{
      console.log(err)
  ...
  const deleteAbout = (refkey:string)=>{
    deleteDoc(doc(db,"Website/AboutPage/Contents",refkey)).then(()=>{
      getAbouts()
    }).catch((err)=>{
      console.log(err)
    })
  ...
  return (
    <Dashboard>
        <p className='text-2xl font-bold text-primary'>About</p>
