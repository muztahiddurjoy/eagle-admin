...
const Gallery = () => {
  ...
  const updateImage =  (file:any,key:string,setloading:Function) =>{
    const storageRef = ref(storage, `Website/Gallery/Images/${key}`)
    setloading(true)
    uploadBytes(storageRef, file).then((snapshot) => {
        getDownloadURL(snapshot.ref).then((url)=>{
            updateDoc(doc(db,"Website/Gallery/Images",key),{
                image: url
            }).then(()=>{
...
const deleteImage = (key:string) =>{
    deleteObject(ref(storage, `Website/Gallery/Images/${key}`)).then(()=>{
        deleteDoc(doc(db,"Website/Gallery/Images",key)).then(()=>{
...
const uploadImage = (file:File,setloading:Function) =>{
    setloading(true)
    addDoc(collection(db,"Website/Gallery/Images"),{
      image:''
    }).then((docRef)=>{
      uploadBytes(ref(storage,`Website/Gallery/Images/${docRef.id}`),file).then(()=>{
        getDownloadURL(ref(storage,`Website/Gallery/Images/${docRef.id}`)).then((url)=>{
          updateDoc(docRef,{
            image: url
          }).then(()=>{
            setloading(false)
  ...
  return (
    <Dashboard>
        <p className='text-2xl font-bold text-primary'>Gallery</p>
