...
const Affiliation = () => {
  ...
  const updateImage =  (file:any,key:string,setloading:Function) =>{
      const storageRef = ref(storage, `Website/Homepage/AffiliationLicense/${key}`)
      setloading(true)
      uploadBytes(storageRef, file).then((snapshot) => {
          getDownloadURL(snapshot.ref).then((url)=>{
              updateDoc(doc(db,"Website/Homepage/AffiliationLicense",key),{
                  image: url
              }).then(()=>{
  ...
  const deleteImage = (key:string) =>{
      deleteObject(ref(storage, `Website/Homepage/AffiliationLicense/${key}`)).then(()=>{
          deleteDoc(doc(db,"Website/Homepage/AffiliationLicense",key)).then(()=>{
  ...
  const uploadImage = (file:File,setloading:Function) =>{
      setloading(true)
      addDoc(collection(db,"Website/Homepage/AffiliationLicense"),{
        image:''
      }).then((docRef)=>{
        uploadBytes(ref(storage,`Website/Homepage/AffiliationLicense/${docRef.id}`),file).then(()=>{
          getDownloadURL(ref(storage,`Website/Homepage/AffiliationLicense/${docRef.id}`)).then((url)=>{
            updateDoc(docRef,{
              image: url
            }).then(()=>{
              setloading(false)
  ...
  return (
    <div>
      <p className="text-primary mt-3 text-2xl font-medium">Affiliation & License</p>
